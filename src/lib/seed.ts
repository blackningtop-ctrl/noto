...
export function createSeedPages(): Page[] {
  ...
  const welcome: Page = {
    ...
    blocks: [
      ...
      block('callout', '💡 Dev Phase 3: Git vault · 정적 docs · 폴더 동기화 · /git 블록 (AI 제외)'),
      ...
      block('todo', '코드 하이라이트 · 복사 · 언어 선택', { checked: true }),
      block('todo', 'Mermaid 다이어그램', { checked: true }),
      block('todo', '커맨드 팔레트 (Ctrl+K)', { checked: true }),
      block('todo', '위키링크 [[페이지]] + 백링크', { checked: true }),
      block('todo', 'ADR / Bug / Sprint / Postmortem 템플릿', { checked: true }),
      block('todo', 'Markdown · JSON 가져오기/내보내기', { checked: true }),
      block('heading2', '시작하기'),
      block('bullet', 'Ctrl+K → 템플릿에서 ADR 또는 Sprint Board 생성'),
      block('bullet', '문장에 [[단축키 & 마크다운]] 처럼 위키링크 작성'),
      block('bullet', '빈 줄에서 /mermaid 또는 /코드'),
      block('quote', '좋은 시스템은 생각을 줄이고, 좋은 노트는 생각을 늘린다.'),
      block('divider', ''),
      block('heading2', '코드 블록'),
      block('code', '// 하이라이트 + 복사 지원\nconst note = { free: true, private: true, dev: true }\nconsole.log(note)', { language: 'typescript' }),
      block('heading2', 'Mermaid'),
